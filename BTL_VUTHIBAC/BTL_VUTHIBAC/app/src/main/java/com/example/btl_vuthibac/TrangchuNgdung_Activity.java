...
ListView lv;
    ArrayList<ThongTinBacSi> mangBSListView; // Danh sách cho ListView

    ThongTinBacSiAdapter adapter;
    ThongTinBacSiAdapter_nguoidung adapterListView;
...
lv=findViewById(R.id.lvngdung);
        // Khởi tạo danh sách và adapter
        mangBSListView = new ArrayList<>(); // Khởi tạo danh sách
        mangBSGridView = new ArrayList<>(); // Khởi tạo danh sác
        // Giả sử bạn muốn hiển thị 3 thuộc tính
        adapter = new ThongTinBacSiAdapter(this, mangBSGridView, false); // false để hiển thị 4 thuộc tính
        grv.setAdapter(adapter);
         adapterListView = new ThongTinBacSiAdapter_nguoidung(this,mangBSListView, false);
        lv.setAdapter(adapterListView);
        database = new Database(this, "datlichkham.db", null, 1);
        viewPager = findViewById(R.id.sl1);
        Loaddulieubacsigridview();
        Loaddulieubacsilistview();
    ...
    private void Loaddulieubacsilistview() {
        Cursor dataCongViec = database.GetData("SELECT bacsi.* FROM bacsi INNER JOIN taikhoan ON bacsi.tendn = taikhoan.tendn order by random() limit 5");
        mangBSListView.clear();
        ...
        while (dataCongViec.moveToNext()) {
            ...
            mangBSListView.add(new ThongTinBacSi(mabs, tenbs, gioitinh, namsinh, sdt, chuyennganh, tdn, blob));
        }

            adapterListView.notifyDataSetChanged(); // Cập nhật adapter
    }
    private void Loaddulieubacsigridview() {
        Cursor dataCongViec = database.GetData("SELECT bacsi.* FROM bacsi INNER JOIN taikhoan ON bacsi.tendn = taikhoan.tendn order by random() limit 4 ");
        mangBSGridView.clear();
        ...
        while (dataCongViec.moveToNext()) {
            ...
            String tdn = dataCongViec.getString(6);
            byte[] blob = dataCongViec.getBlob(7); // Lấy mảng byte từ cột chứa ảnh
            mangBSGridView.add(new ThongTinBacSi(mabs, tenbs, gioitinh, namsinh, sdt, chuyennganh, tdn, blob));
        }
